...
export function registerGatewayMethods(api: OpenClawPluginApi) {
   ...
   * @example
   * ```typescript
   * await gateway.call('dingtalk.sendToUser', {
   *   userId: 'user123',
   *   content: '任务已完成！',
   *   useAICard: true
   * });
   * ```
   */
  api.registerGatewayMethod("dingtalk.sendToUser", async ({ context, params, respond }) => {
    ...
    try {
      ...
      // 构建目标
      const target =
        targetUserIds.length === 1 ? { userId: targetUserIds[0] } : { userIds: targetUserIds };
   ...
   * @example
   * ```typescript
   * await gateway.call('dingtalk.sendToGroup', {
   ...
   * ```
   */
  api.registerGatewayMethod("dingtalk.sendToGroup", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
      const {
  ...
  api.registerGatewayMethod("dingtalk.send", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
      ...
      const account = resolveDingtalkAccount({ cfg, accountId });
      log?.info?.(
        `[Gateway][send] 收到请求: target=${target}, contentLen=${actualContent?.length}`,
      );
  ...
  api.registerGatewayMethod("dingtalk.docs.read", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
   ...
   * @example
   * ```typescript
   * const result = await gateway.call('dingtalk.docs.create', {
   ...
   * ```
   */
  api.registerGatewayMethod("dingtalk.docs.create", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
   ...
   * @example
   * ```typescript
   * await gateway.call('dingtalk.docs.append', {
   *   docId: 'doc123',
   *   content: '补充内容...'
   * });
   * ```
   */
  api.registerGatewayMethod("dingtalk.docs.append", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
   ...
   * @example
   * ```typescript
   * const result = await gateway.call('dingtalk.docs.search', {
   *   keyword: '项目规范',
   ...
   * ```
   */
  api.registerGatewayMethod("dingtalk.docs.search", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
   ...
   * @example
   * ```typescript
   * const result = await gateway.call('dingtalk.docs.list', {
   *   spaceId: 'workspace123',
   ...
   * ```
   */
  api.registerGatewayMethod("dingtalk.docs.list", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
  ...
  api.registerGatewayMethod("dingtalk.status", async ({ context, params, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
      ...
      respond(true, {
        ...
        clientId: hasClientId
          ? String(account.config!.clientId).substring(0, 8) + "..."
          : undefined,
      });
    } catch (err: any) {
  ...
  api.registerGatewayMethod("dingtalk.probe", async ({ context, respond }) => {
    const { loadConfig } = await import("openclaw/plugin-sdk/config-runtime");
    const cfg = loadConfig();
    try {
