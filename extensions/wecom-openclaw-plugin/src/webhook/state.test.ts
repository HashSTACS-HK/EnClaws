...
describe("wecom StreamStore queue", () => {
  test("does not merge into active batch; flushes queued batch after active finishes", async () => {
    ...
    try {
      ...
      const msg1 = { msgid: "M1", msgtype: "text" } satisfies WebhookInboundMessage;
      const msg2 = { msgid: "M2", msgtype: "text" } satisfies WebhookInboundMessage;
      ...
      const r3 = store.addPendingMessage({
        conversationKey,
        target,
        msg: { msgid: "M3", msgtype: "text" } as WebhookInboundMessage,
        msgContent: "3",
        nonce: "n",
        timestamp: "t",
        debounceMs: 10,
  ...
  test("merges into active batch when it has not started yet (even after promotion)", async () => {
    ...
    try {
      ...
      const r1 = store.addPendingMessage({
        conversationKey,
        target,
        msg: { msgid: "M1", msgtype: "text" } as WebhookInboundMessage,
        msgContent: "1",
        nonce: "n",
        timestamp: "t",
        debounceMs: 10,
      ...
      const r2 = store.addPendingMessage({
        conversationKey,
        target,
        msg: { msgid: "M2", msgtype: "text" } as WebhookInboundMessage,
        msgContent: "2",
        nonce: "n",
        timestamp: "t",
        debounceMs: 100,
