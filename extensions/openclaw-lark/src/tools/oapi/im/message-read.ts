...
const GetMessagesSchema = Type.Object({
  ...
  page_size: Type.Optional(
    Type.Number({ description: "每页消息数（1-50），默认 50", minimum: 1, maximum: 50 }),
  ),
      ...
      async execute(_toolCallId: string, params: unknown) {
        ...
        try {
          if (p.open_id && p.chat_id) {
            return json({
              error: "cannot provide both open_id and chat_id, please provide only one",
            });
          }
          if (!p.open_id && !p.chat_id) {
...
const SearchMessagesSchema = Type.Object({
  query: Type.Optional(
    Type.String({ description: "搜索关键词，匹配消息内容。可为空字符串表示不按内容过滤" }),
  ),
...
async function fetchChatContexts(
  ...
  try {
    ...
    logInfo(
      `batch_query: response code=${res.code}, msg=${res.msg}, items=${res.data?.items?.length ?? 0}`,
    );
    if (res.code !== 0) {
...
async function resolveP2PTargetNames(
  ...
  const ids = [
    ...new Set(
      [...chatMap.values()].map((c) => c.p2p_target_id).filter((id): id is string => !!id),
    ),
  ];
  if (ids.length > 0) {
