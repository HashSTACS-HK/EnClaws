...
// Language name → Feishu code language enum
const LANG_MAP = {
  abap: 1,
  ada: 2,
  apache: 3,
...
const DEFAULT_LANG = 1; // plain text

/**
 * Parse inline markdown to Feishu text elements.
 * Handles: **bold**, *italic*, `code`, plain text.
 */
function parseInline(text) {
  const elements = [];
  // Regex: capture bold, italic, code, or plain text chunks
  const re = /\*\*(.+?)\*\*|\*(.+?)\*|`([^`]+)`|([^*`]+)/g;
  let m;
...
/**
 * Convert markdown string to array of Feishu block objects.
 */
...
async function moveToWikiNode(token, documentId, wikiNode) {
  // Extract token from URL if full URL is given
  const nodeToken = wikiNode.includes("/") ? wikiNode.split("/").pop().split("?")[0] : wikiNode;
...
async function main() {
  ...
  // Get valid user access token (auto-refresh if needed)
  let accessToken;
  try {
  ...
  try {
    ...
    // 3. Optionally move to wiki node
    if (args.wikiNode) {
      try {
