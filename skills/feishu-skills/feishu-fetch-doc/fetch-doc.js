 ...
 *   Success: {"doc_id":"...","title":"...","markdown":"...","reply":"..."}
...
/**
 * Extract document token from URL or raw token.
 * Supports: https://xxx.feishu.cn/docx/TOKEN, /wiki/TOKEN, plain TOKEN
 */
...
/** Resolve wiki node token to actual document token */
...
/** Fetch all blocks of a document (handles pagination) */
async function fetchAllBlocks(docId, accessToken) {
  ...
  do {
    ...
    const data = await apiCall(
      "GET",
      `/docx/v1/documents/${docId}/blocks`,
      accessToken,
      null,
      query,
    );
...
function getBlockContent(block) {
  ...
  // Find the content key for this block type
  const keyMap = {
    [BT.PARAGRAPH]: "text",
    [BT.H1]: "heading1",
    [BT.H2]: "heading2",
...
function blockToMarkdown(block) {
  ...
  switch (t) {
    ...
    case BT.PARAGRAPH:
      return elementsToMarkdown(content?.elements);
    case BT.H1:
      return `# ${elementsToMarkdown(content?.elements)}`;
    case BT.H2:
      return `## ${elementsToMarkdown(content?.elements)}`;
    ...
    case BT.BULLET:
      return `- ${elementsToMarkdown(content?.elements)}`;
    case BT.ORDERED:
      return `1. ${elementsToMarkdown(content?.elements)}`;
    case BT.TODO: {
    ...
    case BT.IMAGE: {
      ...
      if (img?.token)
        return `<image token="${img.token}" width="${img.width || ""}" height="${img.height || ""}"/>`;
      return "";
    }
    default:
      return null;
