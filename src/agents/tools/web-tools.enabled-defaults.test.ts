...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "enclaws" }),
                      },
                    },
                  ],
                },
    ...
    const result = await tool?.execute?.("call-1", { query: "latest enclaws release" });
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://enclaws.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://enclaws.ai/docs"]);
