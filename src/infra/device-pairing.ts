...
export async function requestDevicePairing(
  ...
  return await withLock(async () => {
    ...
    if (existing) {
      ...
      await persistState(state, baseDir, tenantContext);
...
export async function rejectDevicePairing(
  ...
  return await withLock(async () => {
    ...
    >({
      requestId,
      idKey: "deviceId",
      loadState: () => loadState(baseDir, tenantContext),
      persistState: (state) => persistState(state, baseDir, tenantContext),
...
export async function verifyDeviceToken(params: {
  ...
  return await withLock(async () => {
    const state = await loadState(params.baseDir, params.tenantContext);
    ...
    state.pairedByDeviceId[device.deviceId] = device;
    await persistState(state, params.baseDir, params.tenantContext);
    return { ok: true };
  });
}
