...
export function mapRoleToGatewayScopes(role: UserRole): string[] {
  switch (role) {
    ...
    case "admin":
      return ["operator.admin", "operator.read", "operator.write", "operator.approvals"];
    case "member":
...
export function mapMethodToPermission(method: string): Permission | null {
  // Agent methods
  if (method === "agents.list" || method === "agent.identity.get") {
    return "agent.list";
  }
  ...
  if (method === "sessions.reset" || method === "sessions.delete") {
    return "session.delete";
  }

  // Config methods
  if (method.startsWith("config.get")) {
    return "config.read";
  }
