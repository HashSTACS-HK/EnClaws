...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "enclaws-cron-failure-alert-"));
  return {
...
describe("CronService failure alerts", () => {
  ...
  it("alerts after configured consecutive failures and honors cooldown", async () => {
    ...
    expect(sendCronFailureAlert).toHaveBeenLastCalledWith(
      expect.objectContaining({
        ...
        text: expect.stringContaining(
          `[CRITICAL SYSTEM ALERT] Your scheduled task "daily report" has failed 2 consecutive times.`,
        ),
      }),
    );
    ...
    expect(sendCronFailureAlert).toHaveBeenLastCalledWith(
      expect.objectContaining({
        text: expect.stringContaining(
          `[CRITICAL SYSTEM ALERT] Your scheduled task "daily report" has failed 4 consecutive times.`,
        ),
        channel: "telegram",
        to: "19098680",
      }),
