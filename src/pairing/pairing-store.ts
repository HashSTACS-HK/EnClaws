...
import { resolveTenantCredentialsDir } from "../config/sessions/tenant-paths.js";
...
function resolveAllowFromPath(
  ...
  if (!normalizedAccountId) {
    return path.join(resolveCredentialsDir(env, tenantContext), `${base}-allowFrom.json`);
  }
  return path.join(
    resolveCredentialsDir(env, tenantContext),
...
export async function readLegacyChannelAllowFromStore(
  ...
  const filePath = resolveAllowFromPath(channel, env, undefined, tenantContext);
...
export async function readChannelAllowFromStore(
  ...
  const scopedPath = resolveAllowFromPath(channel, env, resolvedAccountId, tenantContext);
  ...
  const legacyPath = resolveAllowFromPath(channel, env, undefined, tenantContext);
...
export async function listChannelPairingRequests(
  ...
  const filePath = resolvePairingPath(channel, env, tenantContext);
  return await withFileLock(
    filePath,
...
export async function upsertChannelPairingRequest(params: {
  ...
  const env = params.env ?? process.env;
  const filePath = resolvePairingPath(params.channel, env, params.tenantContext);
  return await withFileLock(
    filePath,
    { version: 1, requests: [] } satisfies PairingStore,
    async () => {
