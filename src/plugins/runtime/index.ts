...
function createRuntimeChannel(): PluginRuntime["channel"] {
  return {
    ...
    pairing: {
      buildPairingReply,
      readAllowFromStore: async ({ channel, accountId, env }) => {
        const tenantContext = await resolveChannelTenantContext(channel, accountId);
        return readChannelAllowFromStore(channel, env, accountId, tenantContext);
      },
      upsertPairingRequest: async ({ channel, id, accountId, meta, env, pairingAdapter }) => {
        const tenantContext = await resolveChannelTenantContext(channel, accountId);
...
function createRuntimeTenant(): PluginRuntime["tenant"] {
  return {
