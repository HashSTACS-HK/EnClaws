...
export async function getSysGatewayConfig(): Promise<SysGatewayConfigRow> {
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.getSysGatewayConfig();
  }
...
export async function upsertSysGatewayConfig(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.upsertSysGatewayConfig(data);
  }
  const sets: string[] = [];
  ...
  if (data.trustedProxies !== undefined) {
    sets.push(`trusted_proxies = $${idx++}`);
    values.push(JSON.stringify(data.trustedProxies));
  }
  if (data.allowRealIpFallback !== undefined) {
    sets.push(`allow_real_ip_fallback = $${idx++}`);
    values.push(data.allowRealIpFallback);
  }
  ...
  if (data.channelHealthCheckMinutes !== undefined) {
    sets.push(`channel_health_check_minutes = $${idx++}`);
    values.push(data.channelHealthCheckMinutes);
  }
  if (data.multiTenant !== undefined) {
    sets.push(`multi_tenant = $${idx++}`);
    values.push(JSON.stringify(data.multiTenant));
  }

  if (sets.length > 0) {
    await query(
      `UPDATE sys_gateway_config SET ${sets.join(", ")}, updated_at = NOW() WHERE id = 1`,
      values,
    );
  }
...
export async function getSysLoggingConfig(): Promise<SysLoggingConfigRow> {
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.getSysLoggingConfig();
  }
...
export async function upsertSysLoggingConfig(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.upsertSysLoggingConfig(data);
  }
  const sets: string[] = [];
  const values: unknown[] = [];
  let idx = 1;
...
export async function getSysPluginsConfig(): Promise<SysPluginsConfigRow> {
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.getSysPluginsConfig();
  }
...
export async function upsertSysPluginsConfig(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.upsertSysPluginsConfig(data);
  }
  const sets: string[] = [];
  const values: unknown[] = [];
  let idx = 1;
  ...
  if (data.entries !== undefined) {
    sets.push(`entries = $${idx++}`);
    values.push(JSON.stringify(data.entries));
  }
  if (data.installs !== undefined) {
    sets.push(`installs = $${idx++}`);
    values.push(JSON.stringify(data.installs));
  }

  if (sets.length > 0) {
    await query(
      `UPDATE sys_plugins_config SET ${sets.join(", ")}, updated_at = NOW() WHERE id = 1`,
      values,
    );
  }
...
export async function getSysToolsConfig(): Promise<SysToolsConfigRow> {
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.getSysToolsConfig();
  }
...
export async function upsertSysToolsConfig(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.upsertSysToolsConfig(data);
  }
  const sets: string[] = [];
  const values: unknown[] = [];
  let idx = 1;
  ...
  if (data.allowDangerousToolsOverride !== undefined) {
    sets.push(`allow_dangerous_tools_override = $${idx++}`);
    values.push(data.allowDangerousToolsOverride);
  }
  if (data.profile !== undefined) {
    sets.push(`profile = $${idx++}`);
  ...
  if (sets.length > 0) {
    await query(
      `UPDATE sys_tools_config SET ${sets.join(", ")}, updated_at = NOW() WHERE id = 1`,
      values,
    );
  }
...
export async function loadAllSysConfig(): Promise<{
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteSysConfig.loadAllSysConfig();
  }
