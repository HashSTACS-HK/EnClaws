...
export async function createTenantChannel(params: {
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.createTenantChannel(params);
  }
...
export async function getTenantChannelById(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.getTenantChannelById(tenantId, channelId);
  }
  const result = await query("SELECT * FROM tenant_channels WHERE tenant_id = $1 AND id = $2", [
    tenantId,
    channelId,
  ]);
...
export async function listTenantChannels(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.listTenantChannels(tenantId, opts);
  }
...
export async function updateTenantChannel(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.updateTenantChannel(tenantId, channelId, updates);
  }
  const sets: string[] = [];
...
export async function deleteTenantChannel(tenantId: string, channelId: string): Promise<boolean> {
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.deleteTenantChannel(tenantId, channelId);
  }
  const result = await query("DELETE FROM tenant_channels WHERE tenant_id = $1 AND id = $2", [
    tenantId,
    channelId,
  ]);
...
export async function toConfigChannels(
  ...
  if (getDbType() === DB_SQLITE) {
    return sqliteChannel.toConfigChannels(channels);
  }
