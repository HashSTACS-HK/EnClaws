...
export function getPlatformSummary() {
  // tenants
  const tenantTotal = Number(
    sqliteQuery(
      `SELECT COUNT(*) as c FROM tenants WHERE status != 'deleted' AND id != '${PLATFORM_TENANT_ID}'`,
    ).rows[0].c,
  );
  const tenantActive = Number(
    sqliteQuery(
      "SELECT COUNT(DISTINCT tenant_id) as c FROM llm_interaction_traces WHERE created_at >= DATE('now', '-30 days')",
    ).rows[0].c,
  );
  ...
  const currentMonth = Number(
    sqliteQuery(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE recorded_at >= DATE('now', 'start of month')",
    ).rows[0].c,
  );
  const lastMonth = Number(
    sqliteQuery(
      "SELECT COALESCE(SUM(input_tokens + output_tokens), 0) as c FROM usage_records WHERE recorded_at >= DATE('now', 'start of month', '-1 month') AND recorded_at < DATE('now', 'start of month')",
    ).rows[0].c,
  );

  // agents
  const agentTotal = Number(sqliteQuery("SELECT COUNT(*) as c FROM tenant_agents").rows[0].c);
  const agentEnabled = Number(
    sqliteQuery("SELECT COUNT(*) as c FROM tenant_agents WHERE is_active = 1").rows[0].c,
  );
  const agentActive = Number(
    sqliteQuery(
      "SELECT COUNT(DISTINCT agent_id) as c FROM llm_interaction_traces WHERE created_at >= DATE('now', '-30 days')",
    ).rows[0].c,
  );
...
export function getLlmStats(period: "all" | "month" | "today") {
  ...
  const turns = Number(
    sqliteQuery(`SELECT COUNT(DISTINCT turn_id) as c FROM llm_interaction_traces WHERE ${cond}`)
      .rows[0].c,
  );

  const avgRow = sqliteQuery(
