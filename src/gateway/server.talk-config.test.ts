...
const TALK_CONFIG_DEVICE_PATH = path.join(
  os.tmpdir(),
  `enclaws-talk-config-device-${process.pid}.json`,
);
...
describe("gateway talk.config", () => {
  it("returns redacted talk config for read scope", async () => {
    ...
    await withServer(async (ws) => {
      ...
      expect(res.payload?.config?.talk?.providers?.elevenlabs?.apiKey).toBe("__ENCLAWS_REDACTED__");
      expect(res.payload?.config?.talk?.voiceId).toBe("voice-123");
      expect(res.payload?.config?.talk?.apiKey).toBe("__ENCLAWS_REDACTED__");
    });
  });
