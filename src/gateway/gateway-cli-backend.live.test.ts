...
const CLI_LIVE = isTruthyEnvValue(process.env.ENCLAWS_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.ENCLAWS_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.ENCLAWS_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("ENCLAWS_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.ENCLAWS_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `ENCLAWS_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const baseCliArgs =
      parseJsonStringArray(
        "ENCLAWS_LIVE_CLI_BACKEND_ARGS",
        process.env.ENCLAWS_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`ENCLAWS_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "ENCLAWS_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.ENCLAWS_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.ENCLAWS_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.ENCLAWS_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "ENCLAWS_LIVE_CLI_BACKEND_IMAGE_MODE requires ENCLAWS_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "enclaws-live-cli-"));
    const disableMcpConfig = process.env.ENCLAWS_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    try {
      ...
      if (previous.skipCanvas === undefined) {
        delete process.env.ENCLAWS_SKIP_CANVAS_HOST;
      } else {
