    ...
    try {
      ...
      if (!client) {
        ...
        if (device && devicePublicKey && !skipPairing) {
          ...
          const paired = await getPairedDevice(device.id, undefined, earlyTenantContext);
        ...
        const nextClient: GatewayWsClient = {
          ...
          rawClientIp: clientIp ?? remoteAddr,
          canvasHostUrl,
          canvasCapability,
          canvasCapabilityExpiresAtMs,
        };

        // --- JWT tenant context resolution ---
        // Reuse early JWT resolution (performed before device pairing above).
        if (earlyTenantContext) {
          ...
          // made during this connection.
          try {
            const ctx = buildRequestContext();
            ctx.resolveTenantCron?.({
              tenantId: earlyTenantContext.tenantId,
              userId: earlyTenantContext.userId,
            });
          } catch {
            // Non-fatal: cron will be lazily initialised on first tool call.
          }
        }
