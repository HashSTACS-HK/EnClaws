...
async function loadTenantAwareSessionEntry(sessionKey: string, tenant?: TenantContext) {
  ...
  const storageKey = toStorageSessionKey(sessionKey, tenantId);
  const canonicalKey = resolveSessionStoreKey({ cfg, sessionKey: storageKey });
  const agentId = resolveSessionAgentIdFromKey(cfg, storageKey);
  const storePath = resolveRequestStorePath(cfg, agentId, tenantId, userId);
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    // In multi-tenant mode, verify the agent exists in DB before proceeding.
    // This prevents auto-creation of agents/main and similar default directories.
    if (tenant?.tenantId && isDbInitialized()) {
      const storageKey = toStorageSessionKey(rawSessionKey, tenant.tenantId);
      ...
      void dispatchInboundMessage({
        ...
        replyOptions: {
          ...
          onAgentRunStart: (runId) => {
            ...
            // Register the agent runId → clientRunId mapping so the agent event
            // handler in server-chat.ts can translate runIds and sessionKeys back
            // to the values the browser client expects.
        ...
        .then(async () => {
          if (!agentRunStarted) {
            ...
            if (combinedReply) {
              const { storePath: latestStorePath, entry: latestEntry } = tenant
                ? await loadTenantAwareSessionEntry(rawSessionKey, tenant)
                : loadSessionEntry(sessionKey);
  ...
  "chat.inject": async ({ params, respond, context, client }) => {
    ...
    const rawSessionKey = p.sessionKey;
    const injectTenant = client?.tenant;
    const { cfg, storePath, entry } = injectTenant
      ? await loadTenantAwareSessionEntry(rawSessionKey, injectTenant)
      : loadSessionEntry(rawSessionKey);
    const sessionId = entry?.sessionId;
