...
export const nodeHandlers: GatewayRequestHandlers = {
  "node.pair.request": async ({ params, respond, context, client }) => {
  ...
  "node.pair.list": async ({ params, respond, client }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      const list = await listNodePairing(undefined, client?.tenant);
  ...
  "node.pair.approve": async ({ params, respond, context, client }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      const approved = await approveNodePairing(requestId, undefined, client?.tenant);
      if (!approved) {
  ...
  "node.pair.reject": async ({ params, respond, context, client }) => {
    ...
    const { requestId } = params as { requestId: string };
    await respondUnavailableOnThrow(respond, async () => {
      const rejected = await rejectNodePairing(requestId, undefined, client?.tenant);
      if (!rejected) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "unknown requestId"));
        return;
      }
  ...
  "node.pair.verify": async ({ params, respond, client }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      const result = await verifyNodeToken(nodeId, token, undefined, client?.tenant);
      respond(true, result, undefined);
    });
  },
  "node.rename": async ({ params, respond, client }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      ...
      const updated = await renamePairedNode(nodeId, trimmed, undefined, client?.tenant);
      if (!updated) {
  ...
  "node.list": async ({ params, respond, context, client }) => {
    ...
    await respondUnavailableOnThrow(respond, async () => {
      const list = await listDevicePairing(undefined, client?.tenant);
  ...
  "node.describe": async ({ params, respond, context, client }) => {
