...
export const deviceHandlers: GatewayRequestHandlers = {
  ...
  "device.pair.approve": async ({ params, respond, context, client }) => {
    ...
    const approved = await approveDevicePairing(requestId, undefined, client?.tenant);
    if (!approved) {
  ...
  "device.pair.reject": async ({ params, respond, context, client }) => {
    ...
    const { requestId } = params as { requestId: string };
    const rejected = await rejectDevicePairing(requestId, undefined, client?.tenant);
    if (!rejected) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "unknown requestId"));
      return;
    }
  ...
  "device.pair.remove": async ({ params, respond, context, client }) => {
    ...
    const removed = await removePairedDevice(deviceId, undefined, client?.tenant);
    if (!removed) {
  ...
  "device.token.revoke": async ({ params, respond, context, client }) => {
    ...
    const entry = await revokeDeviceToken({ deviceId, role, tenantContext: client?.tenant });
    if (!entry) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "unknown deviceId/role"));
      return;
    }
