...
export function renderChatControls(state: AppViewState, tenantAgents?: TenantAgentOption[]) {
  ...
  `;
  const keyMatch = state.sessionKey?.match(/^agent:([^:]+)/);
  const currentAgentId = keyMatch?.[1] ?? "";

  function applySessionKeyChange(next: string) {
  ...
  const effectiveAgentId = currentAgentId || tenantAgents?.[0]?.agentId || "";
    ...
    <div class="chat-controls">
      ${tenantAgents && tenantAgents.length > 0
        ? html`<label class="field chat-controls__session">
            ...
            >
              <option value="" ?selected=${!effectiveAgentId}>选择 Agent...</option>
              ${tenantAgents.map(
                (a) => html`<option value=${a.agentId} ?selected=${a.agentId === effectiveAgentId}>
                  ${a.name || a.agentId}
                </option>`,
              )}
            </select>
...
export function parseSessionKey(key: string): SessionKeyInfo {
  ...
  // ── Web chat session (agent:<x>:chat) ──────────
  if (/^agent:[^:]+:chat$/.test(key)) {
  ...
  // ── Direct chat with account (agent:<x>:<channel>:<account>:direct:<id>) ──
  const directAccountMatch = key.match(/^agent:[^:]+:([^:]+):[^:]+:direct:(.+)$/);
  if (directAccountMatch) {
    const channel = directAccountMatch[1];
    const identifier = directAccountMatch[2];
  ...
  const groupAccountMatch = key.match(/^agent:[^:]+:([^:]+):[^:]+:group:(.+)$/);
  if (groupAccountMatch) {
    const channel = groupAccountMatch[1];
    const channelLabel = CHANNEL_LABELS[channel] ?? capitalize(channel);
